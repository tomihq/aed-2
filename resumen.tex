...
\textbf{Es una algoritmo eficiente no estable. Todos sus casos son de complejidad $O(n \ log \ n)$} \\
...
Es un algoritmo eficiente que está diseñado para ciertos tipos de datos y situaciones muy particulares. \\ \textbf{La estabilidad depende del algoritmo que se use para ordenar los buckets.} \\ \textbf{Los elementos se distribuyen en buckets (cajas) donde cada caja se ordena individualmente, generalmente utilizando otro algoritmo de ordenamiento o aplicando recursivamente bucket sort si es necesario.} \\
...
\textbf{Es un algoritmo de ordenamiento estable no comparativo}. Solo acepta arreglos de números naturales y cuando el rango de los elementos es pequeño en comparación con el número de elementos a ordenar. \\
\textbf{El mejor caso es $O(n+k)$ donde n es el tamaño del arreglo y k es el valor del elemento más grande en el arreglo. \\
...
\end{itemize}
\subsection*{Resumen de complejidades de algoritmos de sorting}\begin{table}[h!]
    \centering
    ...
    \hline 
    Counting Sort & $O(n+k)$ & $O(n+k)$ & $O(n^2)$ & Si \\
    \hline
