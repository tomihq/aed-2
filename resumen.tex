...
Es un algoritmo inestable, su complejidad en el mejor y peor caso es $O(n^{2})$ pues realiza comparaciones cuadráticas sin importar si el arreglo está ordenado o no.
\begin{itemize}
...
Es un algoritmo estable, su complejidad en el mejor caso es de $O(n)$ cuando ya está ordenado y el peor caso es $O(n^{2})$ cuando está completamente desordenado. 
\begin{itemize}
...
Es un algoritmo eficiente y recursivo que sigue el paradigma $divide \ \& \ conquer$ para ordenar una lista de elementos. Es un algoritmo estable, su complejidad en el peor y mejor caso es $O(n \ log \ n)$
\begin{itemize}
...
Es un algoritmo eficiente y recursivo que sigue el paradigma $divide \ \& \ conquer$. Para poder realizarlo necesitamos tener un elemento como pivote. Es un algoritmo no estable, su peor caso es $O(n^{2})$ (eligiendo el peor pivote) y el mejor es $O(n \ log \ n)$
\begin{itemize}
...
Es un algoritmo eficiente que está diseñado para ciertos tipos de datos y situaciones muy particulares. \textbf{Los elementos se distribuyen en buckets (cajas) donde cada caja se ordena individualmente, generalmente utilizando otro algoritmo de ordenamiento o aplicando recursivamente bucket sort si es necesario.} \\
El mejor caso es $O(n+k)$ cuando los elementos están uniformemente distribuidos y con una cantidad parecida en cada bucket. \\
El peor caso es $O(n^{2})$ cuando todos los elementos están en un solo bucket.
\begin{itemize}
...
Es un algoritmo de ordenamiento no comparativo. Solo acepta arreglos de números naturales y cuando el rango de los elementos es pequeño en comparación con el número de elementos a ordenar. \\
El mejor caso es $O(n+k)$ donde n es el tamaño del arreglo y k es el valor del elemento más grande en el arreglo. \\
El peor caso es $O(n^{2})$ cuando todos los elementos están en un solo bucket.
\begin{itemize}
...
Es un algoritmo de ordenamiento no comparativo que reutiliza counting sort procesando las cifras individualmente. Se implementa reutilizando counting sort. Es eficaz cuando se ordenan elementos que tienen claves que se pueden dividir en cifras individuales, como números enteros o cadenas de caracteres numéricos. \\
...
El peor caso es $O(d \ast O(n+k))$ donde d es la cantidad de dígitos y (n+k) es haciendo referencia al counting sort donde n es la longitud del arreglo y k es el valor más grande. \\
...
\begin{lstlisting}
    ...
    Paso 1: Tomo los dígitos de la unidad, y ordeno en base a ellos manteniendo su orden relativo original -> [460, 420, 350, 329]
