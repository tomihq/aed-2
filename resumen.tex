...
\begin{itemize}
    ...
    \item Asignaciones a variables de tipos básicos (las inicializaciones no co0'9nsumen tiempo)
\end{itemize}
...
Básicamente: Tiene que valer en O y en \(\Omega\)
...
\end{itemize}
\subsection*{Diferentes casos de Análisis de Complejidad posibles}
\begin{itemize}
    \item Mejor caso != Peor caso (cuando el ciclo corta antes)
    \item Mejor caso = Peor caso (cuando el ciclo no corta nunca)
\end{itemize}
...
\subsection*{Ejemplo en cálculo de complejidad}
\begin{lstlisting}
...
\end{lstlisting}
Comenzamos colocando la cantidad de operaciones que se realizan, línea a línea. Los ciclos, en el caso del for, posee 3 casos: el caso inicial, caso intermedio, caso fin.
\begin{lstlisting}
    void add(int mat1[n][n], mat2[n][n]){
    0    int ans[n][n];
