...
\begin{itemize}
    ...
    \item Para poder calcular la complejidad, si tenemos una suma basta con quedarnos con el término más grande.
\end{itemize}
...
El ciclo con guarda j <= count depende de una variable count. En el peor caso ¿qué valor toma count? bueno, count se incrementa en el peor caso hasta ser count = |A| (observar el if, que por cada iteración hace el checkeo) por lo tanto el peor caso del while con guarda j <= count. Una observación importante es que count crece igual que |A| en el peor caso, pero el proceso lo hace |A| veces pasando por i = 0, i = 1 SIEMPRE. \\
