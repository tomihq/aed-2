...
\pred{InvRep}{p': PilaConElimArr$<T>$}{
    \longitud{p'.datos} = \longitud{p'.enUso} \land 
    p'.largo = CantidadElementosValidos(p'.enUso)
}
\aux{CantidadElementosValidos}{l: Array$<T>$}{\ent}{
    \sum_{i=0}^{\longitud{l}-1}{if(l[i] \ == \ true) \ then \ 1 \ else \ 0}
}
...
\pred{Abs}{p: PilaConElim$<T>$, p': PilaConElimArr$<T>$}{
    p'.largo = \longitud{p.s} \land
    \paraTodo[unalinea]{i}{\ent}{0\le i<p'.datos \land p'.enUso[i] == true \implicaLuego \existe[unalinea]{j}{\ent}{0 \le j<p.s \land p'.datos[i] = p.s[j]}}
}
...
\begin{center}
    \begin{minipage}[b]{0.8\textwidth}
        \includegraphics[width=\linewidth]{assets/parcial_tad.png}
        \centering
...
\textbf{Demuestre si es verdadero o falso y justifique} $n+m^{5} = O(m^{5})$ \\
...
TODO
\textbf{Demuestre si es verdadero o falso y justifique} $m^{5} = O(n+m^{5})$ \\
...
\textbf{Demuestre si es verdadero, halle un contraejemplo si es falsa y justifique}: $f \in \Omega(n) \implies O(f) \cap \Omega(g) = O(g) \cap \Omega(f)$ \\
Lo primero que puedo ver es que si $ f \in \Omega(g)$ entonces g es una cota inferior de f. \\
Tomo algun ejemplo: $ f = n \land g = n^{3}$ \\
Veo si la fórmula se cumple o si hallamos un posible contraejemplo \\
$\implies O(n) \cap \Omega(n^{3}) = O(n^3) \cap \Omega(n)$  \\
$\implies \varnothing = \{n^{3}, n^{2}, n...\} \cap \{n, n^{2}, n^{3}, ...\}$ \\
$\implies \varnothing = \{n^{3}, n^{2}, n\} $ \\
$\implies False$ \\
